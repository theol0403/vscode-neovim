...
export function* calcDiffWithPosition(oldText: string, newText: string): Generator<DocumentChange> {
    ...
    for (const [start, end, text] of patch) {
        ...
        const charStart = start - (lineStart > 0 ? splitText.slice(0, lineStart).join(eol).length + eol.length : 0);
        const charEnd = end - (lineEnd > 0 ? splitText.slice(0, lineEnd).join(eol).length + eol.length : 0);
        yield {
            start: new Position(lineStart, charStart),
